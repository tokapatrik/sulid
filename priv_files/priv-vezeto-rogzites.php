...
if (count($_REQUEST)>1) { //POST vizsgálat
    if($_REQUEST["vezeteknev"]<>'' && $_REQUEST["keresztnev"]<>'' && $_REQUEST["email"]<>'') { //Megvannak a legszükségesebb adatok?
        ...
        if (count($emailek)==0) { //További feltételek
            ...
            $buildSQLForUserValues=" VALUES ('".$_SESSION["iskola"]["isk_id"]."', '".ucfirst($_REQUEST["vezeteknev"])." ".ucfirst($_REQUEST["keresztnev"]."', 'vez', '").$_REQUEST["email"]."')";
            ...
            $buildSQLForVezetoInsert="INSERT INTO vezetoseg (vez_usr_id, vez_isk_id, vez_nev, vez_nev_vezetek, vez_nev_kereszt, vez_szul_ido, vez_szul_hely, vez_anyja_neve, vez_irsz, vez_varos, vez_cim, vez_email)";
            $buildSQLForVezetoValues=" VALUES ('".$usrId."', '".$_SESSION["iskola"]["isk_id"]."', '".ucfirst($_REQUEST["vezeteknev"])." ".ucfirst($_REQUEST["keresztnev"])."', '".ucfirst($_REQUEST["vezeteknev"])."', '".ucfirst($_REQUEST["keresztnev"])."', '".$_REQUEST["szulido"]."', '".$_REQUEST["szulhely"]."', '".$_REQUEST["anyjaneve"]."', '".$_REQUEST["irsz"]."', '".$_REQUEST["varos"]."', '".$_REQUEST["cim"]."', '".$_REQUEST["email"]."' )";
            setSQL($buildSQLForVezetoInsert.$buildSQLForVezetoValues);
            echo '<div class="ms-5 me-5 mt-3 mb-3 alert alert-success">Vezetőségi tag sikeresen rögzítve!</div>';
        }
        else
        {
            echo '<div class="ms-5 me-5 mt-3 mb-3 alert alert-danger">Már szerepel vezetőségi tag a nyilvántartásban ezekkel az adatokkal!</div>';
        }
    }
    else
    {
...
<div class="p-3 d-flex justify-content-center align-items-center">
    <div class="card shadow w-50">
        <div class="card-header d-flex align-items-center">
            ...
            <h5 class="p-0 m-0">Új vezetőségi tag rögzítése</h5>
        </div>
        <div class="card-body">
            <form method="post">
                ...
                <div class="ms-3">
                    ...
                    <label class="mb-2" style="font-weight: 500;">Vezetőségi tag neve:</label>
                    <div class="d-flex">
                ...
                <button type="submit" class="btn btn-success float-end position-absolute" style="z-index: 10;bottom: 1.25rem; right: 1.25rem;">Veztőségi tag rögzítése</button>
