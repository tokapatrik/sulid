...
if (count($_REQUEST)>1) { //POST vizsgálat
    if($_REQUEST["vezeteknev"]<>'' && $_REQUEST["keresztnev"]<>'' && $_REQUEST["email"]<>'' && $_REQUEST["om"]<>'') { //Megvannak a legszükségesebb adatok?
        $emailek=getSQL("SELECT * FROM user WHERE usr_email='".$_REQUEST["email"]."'");
        $omek=getSQL("SELECT * FROM tanulo WHERE tan_om='".$_REQUEST["om"]."'");
        if (count($emailek)==0 && count($omek)==0) { //További feltételek
            //User beszúr
            $buildSQLForUserInsert="INSERT INTO user (usr_isk_id, usr_nev, usr_tipus, usr_email)";
            $buildSQLForUserValues=" VALUES ('".$_SESSION["iskola"]["isk_id"]."', '".ucfirst($_REQUEST["vezeteknev"])." ".ucfirst($_REQUEST["keresztnev"]."', 'tan', '").$_REQUEST["email"]."')";
            ...
            //Tanuló beszúr
            $buildSQLForTanuloInsert="INSERT INTO tanulo (tan_usr_id, tan_isk_id, tan_nev, tan_nev_vezetek, tan_nev_kereszt, tan_om, tan_szul_ido, tan_szul_hely, tan_anyja_neve, tan_irsz, tan_varos, tan_cim, tan_email)";
            $buildSQLForTanuloValues=" VALUES ('".$usrId."', '".$_SESSION["iskola"]["isk_id"]."', '".ucfirst($_REQUEST["vezeteknev"])." ".ucfirst($_REQUEST["keresztnev"])."', '".ucfirst($_REQUEST["vezeteknev"])."', '".ucfirst($_REQUEST["keresztnev"])."', '".$_REQUEST["om"]."', '".$_REQUEST["szulido"]."', '".$_REQUEST["szulhely"]."', '".$_REQUEST["anyjaneve"]."', '".$_REQUEST["irsz"]."', '".$_REQUEST["varos"]."', '".$_REQUEST["cim"]."', '".$_REQUEST["email"]."' )";
            setSQL($buildSQLForTanuloInsert.$buildSQLForTanuloValues);
            echo '<div class="ms-5 me-5 mt-3 mb-3 alert alert-success">Tanuló sikeresen rögzítve!</div>';
        }
        else
        {
            echo '<div class="ms-5 me-5 mt-3 mb-3 alert alert-danger">Már szerepel tanuló a nyilvántartásban ezekkel az adatokkal!</div>';
        }
    }
    else
    {
...
<div class="p-3 d-flex justify-content-center align-items-center">
    <div class="card shadow w-50">
        <div class="card-header d-flex align-items-center">
            ...
            <h5 class="p-0 m-0">Új tanuló rögzítése</h5>
        </div>
        <div class="card-body">
            <form method="post">
                ...
                <div class="ms-3">
                    ...
                    <label class="mb-2" style="font-weight: 500;">Tanuló neve:</label>
                    <div class="d-flex">
                    ...
                    <label class="mt-3 mb-2" style="font-weight: 500;">OM azonosító:</label>
                    <div class="form-floating ms-1 me-1 ">
                        <input type="text" class="form-control" id="om" name="om" value="<?echo $_REQUEST["om"];?>" placeholder="om">
                        <label for="om">OM</label>
                    </div>
                    <div class="form-text">Az OM azonosító minden tanuló egyedi azonosító kódja</div>   
                </div>
                ...
                <button type="submit" class="btn btn-success float-end position-absolute" style="z-index: 10;bottom: 1.25rem; right: 1.25rem;">Tanuló rögzítése</button>
